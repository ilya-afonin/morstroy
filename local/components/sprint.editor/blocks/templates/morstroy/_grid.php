...
<? } else { ?>
  ...
  switch ($layout['columns'][0]['css']) {
    ...
    case 'file_link': // название сетки
      ...
      <div class="page__content">
        <form class="download-file" action="<?= $content[0]['files'][0]['file']['SRC']; ?>">
        ...
        <table class="info-table">
          ...
          <tr>
            ...
            foreach ($thead as $h => $header):?>
              <td<?
              if ($h == 0) echo ' id="start"'; elseif ($h == count($thead) - 1) echo ' id="end"'; ?>><?= $header['text'] ?></td>
            <?endforeach; ?>
          </tr>
          </thead>
          <tbody>
